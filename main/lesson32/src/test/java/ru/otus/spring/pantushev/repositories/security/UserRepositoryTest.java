...
import ru.otus.spring.pantushev.entities.security.Authority;
...
class UserRepositoryTest {
    ...
    void shouldReturnGetAll3Users() {
        ...
        ua.setAuthorities(Arrays.asList(
            ...
            new Authority(ua.getUsername(), "ROLE_USER_WRITER", ua),
            new Authority(ua.getUsername(), "ROLE_USER_READER", ua)
        ));

        User u1 = new User("User1", "123", (byte) 1);
        u1.setAuthorities(Arrays.asList(
            new Authority(ua.getUsername(), "ROLE_USER_WRITER", u1),
            new Authority(ua.getUsername(), "ROLE_USER_READER", u1)
        ));

        User u2 = new User("User2", "123", (byte) 1);
        u2.setAuthorities(List.of(
            new Authority(ua.getUsername(), "ROLE_USER_READER", u2)
        ));
    ...
    void shouldToString() {
        Iterable<User> actualList = userRepository.findAll();
        actualList.forEach(o -> System.out.println(o));
    }
}
