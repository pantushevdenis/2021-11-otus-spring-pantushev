...
public class ShellApplication {
    ...
    public void showAuthors() {
        Iterable<Author> la = cds.getAuthorAll();
    ...
    public void showJenres() {
        Iterable<Jenre> la = cds.getJenreAll();
    ...
    public void showBook(long id) {
        Optional<Book> b = cds.getBookById(id);
    ...
    public void insertBook(String name, long authorId, long jenreId, @ShellOption(defaultValue = ShellOption.NULL) Integer publishingYear) {
        Book b = new Book(
                name,
                cds.getAuthorById(authorId).orElse(null),
                cds.getJenreById(jenreId).orElse(null),
                publishingYear
        );
        Book inserted = cds.saveBook(b);
    ...
    @ShellMethod("find all books for author")
    public void findBookAuthor(String name) {
        Iterable<Book> books = cds.findBooksByAuthorLike("%" + name + "%");
        pts.printTable(books, Book.class);
    }

    @ShellMethod("find all books for jenres")
    public void findBookJenre(String name) {
        Iterable<Book> books = cds.findBooksByJenreLike("%" + name + "%");
        pts.printTable(books, Book.class);
    }
